module.exports = function (API_KEY, deals, activities, app, type, pbl, employment) {
    ...
    activities.forEach(function(activity) {
        if (activity.subject === 'Paystubs') {
            haves[activity.deal_id] = activity; 
        }   
    }); 

    console.log('Deals that have a paystubs activity:');
    ...
    deals.forEach(function(deal) {
        if (deal.stage_id === app && deal[type.key] === type.refi && deal[employment.key] === employment.employed ||  
            ...
            deal.stage_id === app && deal[type.key] === type.prequal && deal[pbl.key] === pbl.no && deal[employment.key] === employment.employed) {
                queue[deal.id] = deal; 
        }   
    }); 
    ...
    console.log('Deals that need a paystubs activity:');
    console.log(needs);
    ...
    needs.forEach(function(deal){
        request.post('https://api.pipedrive.com/v1/activities?api_token=' + API_KEY, {
            form: {'subject': 'Paystubs',
                   'deal_id': deal,
                     'type' : 'task',
                     'note' : 'Get most recent 30 days of paystubs for ' + queue[deal].person_id.name + '.',
